...
def Euler(xi: float, yi: float, h: float, yprime: Callable[[float, float], float]) -> float:
    return yi + h * yprime(xi, yi)


def ImprovedEuler(xi: float, yi: float, h: float, yprime: Callable[[float, float], float]) -> float:
    k1 = yprime(xi, yi)
    k2 = yprime(xi + h, yi + h * k1)
    return yi + h / 2. * (k1 + k2)


def RungeKutta(xi: float, yi: float, h: float, yprime: Callable[[float, float], float]) -> float:
    k1 = yprime(xi, yi)
    xz = xi + h / 2.
